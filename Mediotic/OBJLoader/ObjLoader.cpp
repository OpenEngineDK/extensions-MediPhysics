...
#ifndef _WIN32
/** 
 * Inserted by Tic because itoa is NOT part of ANSI-C++
...
void ObjLoader::grow(float factor)
{
	...
	vector<Medi::Face>::iterator fi;  // faces
...
void ObjLoader::interpolateNormals()
{
	...
	for(unsigned int i=0;i<newNormal.size();i++)
	{
...
/* Binary STL, http://www.ennex.com/~fabbers/StL.asp */
	...
	for(unsigned long i=0; i<numberOfFacets; i++)
	{
		...
		f.push_back(Medi::Face(v.size()-1,0,vn.size(),v.size(),0,vn.size(),v.size()-2,0,vn.size()));
